...
/*
   ...
   --------------------------
   1. Set your Wi-Fi name and password.
   2. Set ws_host to your deployed backend hostname only
      (no https:// or wss:// prefix).
   3. Set ws_device_secret to the same value as DEVICE_WS_SECRET on the backend.
   4. Keep ws_port=443 and ws_use_tls=true for hosted public deployments.

   Example:
     ws_host = "eduguard-api.onrender.com"
*/
...
/*
   CA certificate for Let's Encrypt ISRG Root X1.
   Render-managed TLS commonly chains to this root.
   If you move to a provider with a different root CA,
   replace this certificate accordingly.
*/
static const char ws_ca_cert[] PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----
...
void webSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
    if (type == WStype_CONNECTED) {
        ...
        snprintf(jsonBuf, sizeof(jsonBuf),
          "{\"type\":\"register\",\"device\":\"CLASSROOM-%s\",\"secret\":\"%s\"}",
          cfgClassroom,
          ws_device_secret);
        webSocket.sendTXT(jsonBuf);
    }
...
void setup() {
    ...
    /* WebSocket */
    if (ws_use_tls) {
        Serial.printf("[WS] Connecting securely to wss://%s%s\n", ws_host, ws_path);
